...
Guidance for automation agents hacking on this repository. The full spec lives in the code—use this file as the quick brief.

## Project Overview
- Rust time-decaying Bloom filter with optional persistence (Fjall primary, ReDB legacy).
- API targets: simple `BloomFilter` core + higher-level expiring filter built on top.
...
- [ ] add Criterion benchmarks covering core Bloom filter persistence with Fjall
      snapshots (save/load throughput + chunk stats).
