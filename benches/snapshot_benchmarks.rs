...
fn temp_db_path(prefix: &str) -> PathBuf {
    use rand::random;
    PathBuf::from(format!("{}_{}.redb", prefix, random::<u64>()))
}
...
fn create_test_filter(db_path: PathBuf, capacity: usize) -> RedbFilter {
    ...
    RedbFilter::new(redb_config).expect("Failed to create RedbFilter")
}
...
fn bench_snapshots(c: &mut Criterion) {
    ...
    for capacity in [1_000, 10_000] {
        ...
        let db_path = temp_db_path(&format!("snapshot_bench_{}", capacity));
...
fn bench_snapshot_fill_levels(c: &mut Criterion) {
    let mut group = c.benchmark_group("snapshot_fill_levels");
    ...
    for fill_percentage in [25, 50, 75, 100] {
        ...
        let db_path = temp_db_path(&format!("snapshot_fill_{}", fill_percentage));
...
fn bench_multi_level_snapshots(c: &mut Criterion) {
    let mut group = c.benchmark_group("multi_level_snapshots");
    group.sample_size(10);
    ...
    for level_count in [1, 2, 3, 5] {
        let db_path = temp_db_path(&format!("snapshot_levels_{}", level_count));
...
criterion_group!(
    benches,
    bench_snapshots,
    bench_snapshot_fill_levels,
    bench_multi_level_snapshots
);
